...
let gradeMaleTotal = male.reduce((acc, cv) => {
    acc = acc + cv.grade;
    return acc;
}, 0);
let averageMaleGrade = gradeMaleTotal / male.length;

// Find the average grade of female

let female = persons.filter((person) => person.sex === "F");
let gradeFemaleTotal = female.reduce((acc, cv) => {
    acc = acc + cv.grade;
    return acc;
}, 0);
let averageFemaleGrade = gradeFemaleTotal / female.length;
...
[...male].sort((a, b) => a.grade - b.grade).pop();

// Find the highest grade in female

[...female].sort((a, b) => a.grade - b.grade).pop();

// Find the highest grade for people whose name starts with 'J' or 'P'

let nameGrade = persons
    .filter(
